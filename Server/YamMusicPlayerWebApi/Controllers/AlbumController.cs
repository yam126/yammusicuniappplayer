...
namespace YamMusicPlayerWebApi.Controllers
{
    ...
    public class AlbumController : Controller
    {
        ...
        /// 获取专辑信息
        /// </summary>
        /// <param name="albumId">专辑信息</param>
        /// <returns>专辑信息</returns>
        [HttpGet]
        [Route("GetAlbumInfo")]
        public EntityResult<albuminfoResult> GetAlbumInfo(string albumId)
        {
            #region 声明变量

            //专辑返回信息
            List<albuminfoResult> resultList = new List<albuminfoResult>();

            //专辑信息
            List<albuminfo> albuminfo = new List<albuminfo>();

            //专辑信息
            var result = new EntityResult<albuminfoResult>();
            ...
            #region 读取数据
            albuminfo = new albuminfo_BLL().Query($" albumId='{albumId}' ", out message);
            if (albuminfo == null || albuminfo.Count <= 0)
            {
                if (!string.IsNullOrEmpty(message))
                    message = $"读取专辑数据出错,原因[{message}]";
                else
                    message = $"读取专辑数据出错,原因[专辑数据不存在]";
                result.Status = -1;
                result.Msg = message;
                return result;
            }
            ...
            #region 循环处理返回值数据
            for (var i = 0; i < albuminfo.Count; i++)
            {
                albuminfo[i].albumCover = $"{PublicFunction.GetRequestHost(Request)}/{albuminfo[i].albumCover}";
                #region 读取创建人信息
                string tempMessage = string.Empty;
                var createrUser = new userinfo_BLL().Query($" userid='{albuminfo[i].userid.GetValueOrDefault().ToString()}' ", out tempMessage);
                string createrUserName = string.Empty;
                string createrUserFace = string.Empty;
                string createrRegistedTime = string.Empty;
                string createrSignature = string.Empty;
                if (createrUser != null && createrUser.Count > 0)
                {
                    createrUserName = createrUser[0].username;
                    string host = PublicFunction.GetRequestHost(Request);
                    if (!string.IsNullOrEmpty(createrUser[0].userface))
                    {
                        if(createrUser[0].userface.IndexOf("http")==-1&& createrUser[0].userface.IndexOf("https") == -1)
                            createrUserFace = $"{host}/{createrUser[0].userface}";
                        else
                            createrUserFace = createrUser[0].userface;
                    }
                    createrRegistedTime = createrUser[0].createdatetime.GetValueOrDefault().ToString("yyyy-MM-dd HH:mm:ss");
                    createrSignature = createrUser[0].signature;
                }
                #endregion
                resultList.Add(new albuminfoResult()
                {
                    albumAuthor = albuminfo[i].albumAuthor,
                    albumCover = albuminfo[i].albumCover,
                    albumId = albuminfo[i].albumId.GetValueOrDefault().ToString(),
                    albumIntro = albuminfo[i].albumIntro,
                    albumName = albuminfo[i].albumName,
                    createdatetime = albuminfo[i].createdatetime.GetValueOrDefault().ToString("yyyy-MM-dd hh:mm:ss"),
                    modifieddatetime = albuminfo[i].modifieddatetime.GetValueOrDefault().ToString("yyyy-MM-dd hh:mm:ss"),
                    modifierId = albuminfo[i].modifierId.GetValueOrDefault().ToString(),
                    shareCount = albuminfo[i].shareCount.GetValueOrDefault().ToString(),
                    songLength = albuminfo[i].songLength.GetValueOrDefault().ToString(),
                    userid = albuminfo[i].userid.GetValueOrDefault().ToString(),
                    userFace = createrUserFace,
                    userName = createrUserName,
                    RegistedTime = createrRegistedTime,
                    Signature = createrSignature,
                    viewCount = albuminfo[i].viewCount.GetValueOrDefault().ToString()
                });
            }
            #endregion
        ...
        [HttpPost]
        [Route("EditAsync")]
        public async Task<EntityResult<albuminfoResult>> EditAlbumAsync()
        {
            #region 声明变量
            ...
            {
                result = new EntityResult<albuminfoResult>()
                {
                    Status = -1,
            ...
            result = new EntityResult<albuminfoResult>()
            {
                Status = 0,
                Msg = string.Empty,
                Result = PublicFunction.ConvertApi(saveAlbumsData, Request)
            };
            return result;
        }
