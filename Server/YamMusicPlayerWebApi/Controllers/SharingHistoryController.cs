...
namespace YamMusicPlayerWebApi.Controllers
{
    ...
    public class SharingHistoryController : Controller
    {
        ...
        [HttpPost]
        [Route("Add/Album")]
        public Result AddShareAlbum(string albumId) 
        {
            #region 声明变量
            ...
            //专辑业务类
            albuminfo_BLL bll=new albuminfo_BLL();
            ...
            albums = bll.Query($" albumId='{albumId}' ", out message);
            if(albums==null || albums.Count<=0) 
            {
                if (!string.IsNullOrEmpty(message))
                    message = $@"读取专辑信息出错,原因[{message}]";
                else
                    message = "读取专辑信息出错,原因[没有读取到任何专辑信息]";
                result = new Result() 
                {
                    Status=-1,
                    Msg=message
            ...
            albums[0].shareCount += 1;
            State = bll.Update(albums, $" albumId='{albumId}' ", out message);
            if (State != 0) 
            {
                result = new Result()
                {
                    Status = -1,
                    Msg = $"更新专辑分享次数出错,原因[{message}]"
                };
                return result;
            }
        ...
        private vwSharingHistory ConvertApi(vw_sharinghistory args, HttpRequest Request)
        {
            ...
            bool isFriend = false;
            string ShareTargetUserFace = string.Empty;
            string ShareUserFace = string.Empty;
            ...
            if (!string.IsNullOrEmpty(args.ShareTargetUserFace))
            {
                ShareTargetUserFace = $"{host}/{args.ShareTargetUserFace}";
            }
            if (!string.IsNullOrEmpty(args.ShareUserFace)) 
            {
                ShareUserFace= $"{host}/{args.ShareUserFace}";
            }
