...
namespace YamMusicPlayerWebApi.Controllers
{
    ...
    public class NoAuthController : Controller
    {
        ...
        public EntityResult<LoginResult> Login(string username, string password, string deviceId, int deviceType)
        {
            ...
            string userFaceUrl = dbUserInfo.Userface.ToString();
            if (userFaceUrl.ToLower().IndexOf("http") == -1)
                userFaceUrl = $"{host}/{dbUserInfo.Userface.ToString()}";
            result = new EntityResult<LoginResult>()
            {
                Status = 0,
                Msg = string.Empty,
        ...
        /// 验证登陆状态
        /// </summary>
        /// <param name="argument">登陆状态验证参数</param>
        /// <returns>是否验证通过</returns>
        [HttpPost]
        [Route("CheckLogin")]
        public EntityResult<bool> CheckLogin([FromBody] CheckLoginArgument argument)
        {
            #region 声明变量
